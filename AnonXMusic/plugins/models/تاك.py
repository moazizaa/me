...
@app.on_message(filters.command(["اسمي", "شن اسمي"]) & filters.group )
...
array = []
@app.on_message(filters.command(["@all", "تاغ","تاغ للكل"]) & ~filters.private)
async def nummmm(client: app, message):
  if message.chat.id in array:
     return await message.reply_text(f"**تم بدأ التاق الجماعي \n\n بواسطة ← ✧ ¦{message.from_user.mention}**")

  dev = (OWNER_ID)
  haya = (1183747742,6195765774)
...
@app.on_message(filters.command(["وقف منشن", "/cancel","خلاص"]))
